...
//    Your job is to invent a smarter malloc algorithm than the my malloc.
...
void insert_metadata(my_metadata_t *prev, my_metadata_t *metadata) {
  assert(!metadata->next);
  metadata->next = prev->next;
  prev->next = metadata;
}

void insert_metadata_to_head(my_metadata_t *metadata) {
  assert(!metadata->next);
  metadata->next = my_heap.free_head;
  my_heap.free_head = metadata;
}

my_metadata_t *find_prev_metadata(size_t size) {
...
void my_add_to_free_list(my_metadata_t *metadata) {
  assert(!metadata->next);
  my_metadata_t *prev = find_prev_metadata(metadata->size);
  if (!prev) {
    insert_metadata_to_head(metadata);
  } else {
    insert_metadata(prev, metadata);
  }
}
...
void *my_malloc(size_t size) {
  fprintf(stderr, "search size: %zu", size);
  my_metadata_t *target = NULL;
  my_metadata_t *prev = NULL;
  my_metadata_t *metadata = my_heap.free_head;

  while (metadata && metadata->size >= size) {
    fprintf(stderr, "%zu   ", metadata->size);
    prev = target;
    target = metadata;
    metadata = metadata->next;
  }
  if (!target) {
    fprintf(stderr, "fetch new memory\n");
  ...
  my_remove_from_free_list(target, prev);
